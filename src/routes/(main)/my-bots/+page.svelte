...
<p class="pt-2 pb-2 text-sm text-neutral-400">
	You may have at most 3 active bots at once
...
{#if !bots || bots.length === 0}
	<div class="pt-4">
		...
		<a href="/submit-bot" class="btn bg-blue-500 hover:bg-blue-600 active:bg-blue-400">
...
{:else}
		...
		{#each bots as bot (bot.id)}
			<li class="rounded-lg border border-zinc-700 bg-zinc-900 p-4">
				<div class="flex items-start justify-between gap-4">
					<div>
						<h2 class="font-semibold">
							<a href="/bot/{bot.id}" class="hover:text-blue-400">{bot.name}</a>
						</h2>
					...
					{#if bot.active}
						<span class="rounded bg-green-600 px-2 py-0.5 text-xs font-bold">Active</span>
					{:else}
						<span class="rounded bg-zinc-700 px-2 py-0.5 text-xs font-bold">Inactive</span>
					{/if}
				</div>
				...
				<div class="flex items-center gap-4 pt-3">
					<a href="/bot/{bot.id}" class="text-sm text-blue-400 hover:underline">
						View
					</a>

					<form {...toggleActiveForm} class="inline-flex items-center gap-2">
						<input {...toggleActiveForm.fields.id.as("hidden", bot.id)}>
						<label class="pb-0!">
							<input {...toggleActiveForm.fields.active.as("checkbox")} checked={bot.active}>
							<span class="pl-2">{bot.active ? "Deactivate" : "Activate"}</span>
						</label>
						<button class="btn bg-blue-500 hover:bg-blue-600 active:bg-blue-400 text-sm px-3 py-1" type="submit">Save</button>
					</form>
				</div>
			</li>
		{/each}
	...
	<p class="pt-6">
		<a href="/submit-bot" class="text-blue-400 hover:underline">Submit a new bot</a>
	</p>
{/if}